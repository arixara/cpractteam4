...
int main() {
	...
	for (int b = 0; b < n; b++) { // для выбранного количества запусков
		...
		std::cout << "Укажите количество потоков (выпонлять 1 или 2 задачи одновременно)\n";
		cin >> cores;
		...
		//разделить на потоки
		 //открыть файл номер way, засечь время
		int time = reading(way);


		cout << "\nЗапуск номер " << b + 1 << " выполнялся на файле № " << way << ", задействовано " << cores << " потоков при " << gf << " логических ядрах. \nВыполнялся в течение " << time << " милисекунд.\n\n";
		resf << b + 1 << endl;
		resf << way << endl;
		resf << cores << endl;
		resf << gf << endl;
